...
<html>
			...
			<table class="easyui-datagrid" id="dom_datagridfarmqzscheduler">
				<thead>
					<tr>
						...
						<th field="RUN" data-options="sortable:false" width="80">
							是否启动
						</th>
					</tr>
				</thead>
	...
	var url_delActionfarmqzscheduler = "admin/FarmQzSchedulerdeleteCommit.do";//删除URL
	var url_formActionfarmqzscheduler = "admin/FarmQzSchedulershow.do";//增加、修改、查看URL
	var url_searchActionfarmqzscheduler = "admin/FarmQzSchedulerqueryAll.do";//查询URL
	...
	function viewDatafarmqzscheduler() {
		...
		if (selectedArray.length > 0) {
			var url = url_formActionfarmqzscheduler + '?pageset.pageType='
	...
	function runDatafarmqzscheduler(){
		...
		if (selectedArray.length > 0) {
			...
			$.messager.confirm(MESSAGE_PLAT.PROMPT, str, function(flag) {
				if (flag) {
					$.post("admin/FarmQzRunOnecTask.do" + '?ids='
					...
					.confirm(
							...
							function(flag) {
								if (flag) {
									$
											.post(
													'admin/FarmQzSchedulereditStart.do' + '?ids=' + $.farm
															.getCheckedIds(gridfarmqzscheduler),
													{},
													function(flag) {
														if (flag.pageset.commitType == 0) {
															$(
					...
					.confirm(
							...
							function(flag) {
								if (flag) {
									$
											.post(
													'admin/FarmQzSchedulereditStop.do' + '?ids=' + $.farm
															.getCheckedIds(gridfarmqzscheduler),
													{},
													function(flag) {
														if (flag.pageset.commitType == 0) {
