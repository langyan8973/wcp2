...
<script type="text/javascript">
	var url_delActiontreeType = "admin/ALONEDictionaryType_DEL_SUBMIT.do";//删除URL
	var url_formActiontreeType = "admin/ALONEDictionaryType_VIEWTREE_SUBMIT.do";//增加、修改、查看URL
	var url_searchActiontreeType = "admin/ALONEDictionaryType_SYSBASE_QUERYALL.do?ids=${ids}";//查询URL
		...
		$('#tt').tree( {
			...
			onSelect : function(node) {
				...
				searchtreeType.dosearch( {
					'query.ruleText' : searchtreeType.arrayStr()
				});
			}
		});
	...
	function viewDatatreeType() {
		...
		if (selectedArray.length == 1) {
			var url = url_formActiontreeType + '?pageset.pageType='
	...
	function addDatatreeType() {
		var url = url_formActiontreeType + '?dicId=${ids}&pageset.pageType='
				+ PAGETYPE.ADD;
		$.farm.openWindow( {
			id : 'wintreeType',
			width : 600,
	...
	function addSubDatatreeType() {
		...
		if (selectedArray.length > 0) {
			var url = url_formActiontreeType + '?dicId=${ids}&parentId='
					+ selectedArray[0].ID + '&pageset.pageType=' + PAGETYPE.ADD;
			$.farm.openWindow( {
				id : 'wintreeType',
				width : 600,
				height : 300,
