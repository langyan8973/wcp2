...
<html>
	...
	var url_delActionOrganization = "admin/FarmAuthorityOrganizationDeleteCommit.do";//删除URL
	var url_formActionOrganization = "admin/FarmAuthorityOrganizationTabs.do";//增加、修改、查看URL
	var url_searchActionOrganization = "admin/FarmAuthorityOrganizationQuery.do";//查询URL
		...
		$('#OrganizationTree').tree( {
			...
			onSelect : function(node) {
				...
				searchOrganization.dosearch( {
					'query.ruleText' : searchOrganization.arrayStr()
				});
			}
		});
	...
	function viewDataOrganization() {
		...
		if (selectedArray.length == 1) {
			var url = url_formActionOrganization + '?pageset.pageType='
	...
	function addDataOrganization() {
		var url = url_formActionOrganization + '?parent.id='
	...
	function moveOrgtree() {
		...
		if (selectedArray.length > 0) {
			$.farm.openWindow( {
				...
				height : 300,
				modal : true,
				url : "admin/farmAuthorityOrgTreeNodeTreeView.do?ids="
						+ $.farm.getCheckedIds(gridOrganization, 'ID'),
				title : '移动机构'
			});
		} else {
