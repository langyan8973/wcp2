...
<script type="text/javascript">
	var url_delActionDocgroupuser = "admin/FarmDocgroupUserdeleteCommit.do";//删除URL
	var url_formActionDocgroupuser = "admin/FarmDocDocgroupuserShow.do";//增加、修改、查看URL
	var url_searchActionDocgroupuser = "admin/FarmDocgroupUserqueryAll.do?ids=${ids}";//查询URL
			...
			$.messager.confirm(MESSAGE_PLAT.PROMPT, "是否设置管理员权限？",
					function(flag) {
						if (flag) {
							...
							$.post("admin/FarmDocgroupUserSetAdmin.do"
									+ '?ids='
		...
		if (selectedArray.length > 0) {
			...
			$.messager.confirm(MESSAGE_PLAT.PROMPT, "是否取消管理员权限？",
					function(flag) {
						if (flag) {
							$(gridDocgroupuser).datagrid('loading');
							$.post("admin/FarmDocgroupUserResetAdmin.do"
									+ '?ids='
									+ $.farm.getCheckedIds(gridDocgroupuser,
											'ID'), {}, function(flag) {
								$(gridDocgroupuser).datagrid('loaded');
	...
	function addDataDocgroupuser() {
		...
		chooseWindowCallBackHandle = function(row) {
			...
			$.post("admin/FarmDocgroupUseraddCommit.do", {
				'userids' : userids,
				ids : '${ids}'
			}, function(flag) {
				if (flag.pageset.commitType == 0) {
					...
					$.messager.alert(MESSAGE_PLAT.ERROR, flag.pageset.message,
							'error');
				}
			});
		};
	...
	function editDataDocgroupuser() {
		...
		if (selectedArray.length == 1) {
			var url = url_formActionDocgroupuser + '?pageset.pageType='
