...
<html>
	...
	var url_searchActionFarmdoc = "admin/FarmDocqueryAll.do";//查询URL
		...
		$('#FarmdocTree').tree( {
			...
			onSelect : function(node) {
				...
				searchFarmdoc.dosearch( {
					'query.ruleText' : searchFarmdoc.arrayStr()
				});
			}
		});
	...
	function commentMng() {
		...
		if (selectedArray.length == 1) {
			$.farm.openWindow( {
				...
				height : 300,
				modal : true,
				url : "admin/FarmDocCommentMng.do" + '?pageset.pageType=2&ids='
						+ selectedArray[0].ID,
				title : '评论'
			});
		} else {
	...
	function viewDataFarmdoc() {
		var selectedArray = $(gridFarmdoc).datagrid('getSelections');
		if (selectedArray.length == 1) {
			var url = url_formActionFarmdoc + '?pageset.pageType='
	...
	function addDataFarmdoc() {
		var url = url_formActionFarmdoc + '?doctype.id='
	...
	function stttionPop() {
		...
		if (selectedArray.length >0) {
			$.farm.openWindow( {
				...
				height : 200,
				modal : true,
				url : "admin/FarmDocRightedit.do" + '?pageset.pageType=2&ids='
						+ selectedArray[0].ID,
				title : '批量设置权限'
			});
		} else {
