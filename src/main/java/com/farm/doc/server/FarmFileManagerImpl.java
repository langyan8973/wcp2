...
public class FarmFileManagerImpl implements FarmFileManagerInter {
	...
	private static final Logger log = Logger
			.getLogger(FarmFileManagerImpl.class);

	@Override
	public String saveFile(File file, FILE_TYPE type, String title,
			LoginUser user) {
		...
		FarmDocfile docfile = new FarmDocfile(FarmDocFiles.generateDir(), UUID
				...
				title, file.getName(), TimeTool.getTimeDate14(), TimeTool
						.getTimeDate14(), userName, userId, userName, userId,
				"0", null, exName, Float.valueOf(String.valueOf(file.length())));
		if (user == null || user.getName() == null) {
			docfile.setCusername("none");
			docfile.setEusername("none");
		}
		FarmDocFiles.copyFile(file, FarmDocFiles.getFileDirPath()
				+ docfile.getDir());
	...
	public String saveFile(InputStream inStream, String filename, String title,
			FILE_TYPE type, LoginUser user) {
		String exName = FarmDocFiles.getExName(title);
		if (exName.trim().toUpperCase().replace(".", "").equals("ZIP")) {
			type = FILE_TYPE.RESOURCE_ZIP;
		}
		...
		FarmDocfile docfile = new FarmDocfile(FarmDocFiles.generateDir(), UUID
				.randomUUID().toString().replaceAll("-", ""), type.getValue(),
				title, filename, TimeTool.getTimeDate14(), TimeTool
						.getTimeDate14(), userName, userId, userName, userId,
				"0", null, exName, Float.valueOf(String.valueOf(0)));
		if (user == null || user.getName() == null) {
			docfile.setCusername("none");
			docfile.setEusername("none");
		}
		long length = FarmDocFiles.saveFile(inStream, filename, DocumentConfig
				.getString("config.doc.dir")
				+ docfile.getDir());
	...
	public FarmDocfile getFile(String fileid) {
		...
		file.setFile(new File(FarmDocFiles.getFileDirPath() + File.separator
				+ file.getDir() + file.getFilename()));
		// 如果文件是大小是0的话就刷新文件大小
		if (file.getLen() == 0) {
	...
	public FarmDocfile openFile(String exname, String content, LoginUser user) {
		...
		File file = new File(FarmDocFiles.getFileDirPath() + docfile.getDir()
