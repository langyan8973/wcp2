...
public class ZipUtils {
	...
	public static void unHtmlZipFiles(FarmDocfile zipFile, String descDir)
			throws IOException {
		File pathFile = new File(descDir);
		if (!pathFile.exists()) {
			pathFile.mkdirs();
		}
		ZipFile zip = new ZipFile(zipFile.getFile());
	...
	public static void reCodeFilesUrl_link(File file) throws IOException {
		...
		// 获得文件的编码格式
		if (file.getName().toUpperCase().indexOf(".HTML") > 0
				|| file.getName().toUpperCase().indexOf(".HTM") > 0) {
			...
			for (Element node : metas) {
				if (node.attr("content").toUpperCase().indexOf("CHARSET") >= 0) {
					...
					charSet = node.attr("content").substring(
							node.attr("content").toUpperCase().indexOf(
									"CHARSET") + 8);
					break;
				}
			}
		...
		}
		{
			...
			for (Element node : doc.getElementsByTag("title")) {
				...
				str.append(AppConfig.getString("config.sys.title"));
