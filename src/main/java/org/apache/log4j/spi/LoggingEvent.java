...
public class LoggingEvent implements java.io.Serializable {
	...
	public LoggingEvent(final String fqnOfCategoryClass, final Category logger,
			final long timeStamp, final Level level, final Object message,
			final String threadName, final ThrowableInformation throwable,
			final String ndc, final LocationInfo info,
	...
	@SuppressWarnings("unchecked")
	private void readLevel(ObjectInputStream ois) throws java.io.IOException,
		...
		try {
			...
			if (className == null) {
				...
				Object[] intarray = new Integer[] { new Integer(p) };
				level = (Level) m.invoke(null, intarray);
			}
			...
			LogLog.warn("Level deserialization failed, reverting to default.",
					e);
			level = Level.toLevel(p);
	...
	public LoggingEvent cutMessager(int num) throws Exception {
		LoggingEvent obj = this;
		if (getMessage().toString().length() > num) {
			obj.message = obj.message.toString().substring(0, num) + "...";
		}
		return obj;
	}
}
