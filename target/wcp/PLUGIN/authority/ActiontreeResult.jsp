...
<html>
				...
				<form id="searchActiontreeForm">
					<table class="editTable">
						<tr>
							...
							<td>
								<select name="DOMAIN:=" style="width: 120px;">
									<option value="">
									</option>
									<option value="alone">
										标准权限
	...
	var url_delActionActiontree = "admin/FarmAuthorityActiontreeDeleteCommit.do";//删除URL
	var url_formActionActiontree = "admin/FarmAuthorityActiontreeShow.do";//增加、修改、查看URL
	var url_searchActionActiontree = "admin/FarmAuthorityActiontreeQuery.do";//查询URL
		...
		$('#ActiontreeTree').tree( {
			...
			onSelect : function(node) {
				...
				searchActiontree.dosearch( {
					'query.ruleText' : searchActiontree.arrayStr()
				});
			}
		});
	}
	...
	function viewDataActiontree() {
		...
		if (selectedArray.length == 1) {
			var url = url_formActionActiontree + '?pageset.pageType='
	...
	function addDataActiontree() {
		...
		var url = url_formActionActiontree + '?parent.id='
	...
	function moveActiontree() {
		...
		if (selectedArray.length > 0) {
			$.farm.openWindow( {
				...
				height : 300,
				modal : true,
				url : "admin/farmAuthorityActionTreeNodeTreeView.do?ids="
						+ $.farm.getCheckedIds(gridActiontree, 'ID'),
				title : '移动菜单'
			});
		} else {
