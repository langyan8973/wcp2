...
<html>
	...
	var url_delActionUser = "admin/FarmAuthorityUserdeleteCommit.do";//删除URL
	var url_formActionUser = "admin/FarmAuthorityUserShow.do";//增加、修改、查看URL
	var url_searchActionUser = "admin/FarmAuthorityUserQuery.do";//查询URL
	...
	function viewDataUser() {
		...
		if (selectedArray.length == 1) {
			var url = url_formActionUser + '?pageset.pageType=' + PAGETYPE.VIEW
	...
	function addDataUser() {
		var url = url_formActionUser + '?pageset.pageType=' + PAGETYPE.ADD;
		$.farm.openWindow( {
			id : 'winUser',
			width : 600,
			height : 300,
	...
	function editDataUser() {
		var selectedArray = $(gridUser).datagrid('getSelections');
		if (selectedArray.length == 1) {
			var url = url_formActionUser + '?pageset.pageType=' + PAGETYPE.EDIT
					+ '&ids=' + selectedArray[0].ID;
			$.farm.openWindow( {
				id : 'winUser',
				width : 600,
	...
	function initPasswd() {
		...
		if (selectedArray.length > 0) {
			...
			$.messager.confirm(MESSAGE_PLAT.PROMPT, "该操作将无法返回，是否继续执行密码初始化?",
					function(flag) {
						if (flag) {
							$.post("admin/FarmAuthorityUserInitPassWdCommit.do"
									+ '?ids='
	...
	function setUserPost() {
		...
		if (selectedArray.length > 0) {
			$.farm.openWindow( {
				...
				height : 300,
				modal : true,
				url : "admin/farmAuthorityUserORGTreeView.do?ids="
						+ $.farm.getCheckedIds(gridUser, 'ID'),
				title : '选择组织机构岗位'
			});
		} else {
