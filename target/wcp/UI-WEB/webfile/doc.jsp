...
<div class="row">
	<div class="col-md-8">
		...
		<ol class="breadcrumb">
			<c:forEach var="node" items="${doc.currenttypes}" varStatus="status">
				<li><a href="index/FPtype.htm?id=${node.id}" title="${node.name}">${node.name}</a>
				</li>
			</c:forEach>
		</ol>
	...
	<div class="col-md-4">
		<jsp:include page="/UI-WEB/docgroup/commons/includeGroupLogo.jsp"></jsp:include>
		<jsp:include page="/UI-WEB/lucene/commons/includeLuceneResultMin.jsp"></jsp:include>
	</div>
</div>
		...
		$('#editTopButtonId').bind(
				...
				function() {
					window.location = basePath
							+ 'index/FLeditWebfile.htm?id=${doc.id}';
				});
		$('#docMessageId').bind(
				'click',
				function() {
					window.location = basePath
							+ 'index/FLDocMessageShow.htm?id=${doc.id}';
				});
		$('#delTopButtonId').bind(
				'click',
				function() {
